...
function (_AnimationValue) {
  ...
  _proto._update = function _update(_ref, props, timestamp, resolve) {
    ...
    var goal = parent ? null : computeGoal(to); // Reset our internal `Animated` node if starting.
...
function useTransition(data, props, deps) {
  ...
  var keys = shared.is.und(key) ? items : shared.is.fun(key) ? items.map(key) : shared.toArray(key); // The "onRest" callbacks need a ref to the latest transitions.
...
function () {
  ...
  _proto.advance = function advance(dt, spring) {
    ...
    anim.values.forEach(function (node, i) {
      ...
      var canFinish = !payload || payload[i].done;
